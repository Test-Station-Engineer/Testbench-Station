...
void loop() {
  detectBoardVersion();
  if(board_version_index == BOARD_DETECTED_NONE){
    blink();
  }
...
void processSerialCommand(void){
  ...
  else if(strcmp(tok,"set_load_led")==0){
    tok = strtok(NULL,delim);
    if(tok==NULL){
    }
  ...
  else if(strcmp(tok,"write_com_read_console")==0){
    for(i=23; i<strlen(serial_command_str); i++){
      str_msg[i-23] = serial_command_str[i];
    }
    str_msg[i-23] = 0; // null terminate string
...
void initRS485(void){
  ...
  pinMode(RE_COM,OUTPUT);
  pinMode(DE_COM,OUTPUT);
  setRS485TERMOff();
  setRS485COMOff();
...
void writeCOMReadConsole(char * msg){
